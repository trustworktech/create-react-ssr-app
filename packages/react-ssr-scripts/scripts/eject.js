...
inquirer
  ...
  .then(answer => {
    ...
    const folders = ['config', 'config/jest', 'config/webpack', 'scripts'];
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove react-ssr-scripts reference since they're getting a copy of the types in their project
        content =
          content
            // Remove react-ssr-scripts types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"react-ssr-scripts".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove react-ssr-scripts and react-ssr-scripts binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      const windowsCmdFilePath = path.join(
        ...
        'node_modules',
        '.bin',
        'react-ssr-scripts.cmd'
      );
      let windowsCmdFileContent;
      if (process.platform === 'win32') {
        // https://github.com/facebook/create-react-app/pull/3806#issuecomment-357781035
        // Yarn is diligent about cleaning up after itself, but this causes the react-ssr-scripts.cmd file
