  ...
  if (overrides) {
    ...
    if (unsupportedKeys.length) {
      ...
      if (isOverridingSetupFile) {
        ...
        console.error(
          chalk.red(
            '\nOut of the box, Create React SSR App only supports overriding ' +
              'these Jest options:\n\n' +
              supportedKeys
              ...
              'are not currently supported by Create React SSR App:\n\n' +
              unsupportedKeys
                .map(key => chalk.bold('  \u2022 ' + key))
                .join('\n') +
              ...
              'You may also file an issue with Create React SSR App to discuss ' +
