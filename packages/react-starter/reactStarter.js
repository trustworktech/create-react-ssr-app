...
const program = new commander.Command(packageJson.name)
  ...
  .on('--help', () => {
    ...
    console.log(
      `      ${chalk.cyan(
        'https://github.com/verumtech/react-starter/issues/new'
      )}`
    );
    console.log();
  })
    ...
    console.log(
      chalk.red(
        `You are using Node ${process.version}.\n\n` +
          `Please update to Node 8.10 or higher to use React Starter.\n`
      )
    );
    process.exit(1);
  }
        ...
        console.log(
          chalk.red(
            `You are using npm ${npmInfo.npmVersion}.\n\n` +
              `Please update to npm 5 or higher to use React Starter.\n`
          )
        );
      }
      process.exit(1);
...
function install(root, useYarn, usePnp, dependencies, verbose, isOnline) {
  return new Promise((resolve, reject) => {
    ...
    if (useYarn) {
      ...
      // Explicitly set cwd() to work around issues
    ...
    console.error(
      chalk.red(
        'You are running Node %s.\n' +
          'React Starter requires Node %s or higher. \n' +
...
// We also special case IJ-based products .idea because it integrates with React Starter
