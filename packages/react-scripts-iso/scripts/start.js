...
const clearConsole = require('@verumtech/react-dev-utils/clearConsole');
...
if (process.env.HOST) {
  ...
  console.log(
    `Learn more here: ${chalk.yellow(
      'https://react-starter.dev/docs/advanced-configuration'
    )}`
  );
  console.log();
}
...
const { checkBrowsers } = require('@verumtech/react-dev-utils/browsersHelper');
checkBrowsers(paths.appPath, isInteractive)
  ...
  .then(port => {
    ...
    appUrls = prepareUrls('http', HOST, appPort);
  ...
  .then(port => {
    ...
    devUrls = prepareUrls('http', HOST, devPort);
    // Generate configuration
    const [clientConfig, serverConfig] = configFactory('development');

    clientConfig.entry = [
      `@verumtech/react-dev-utils/webpackHotDevClient?devPort=${devPort}`,
      ...clientConfig.entry,
    ];
    ...
    const clientCompiler = createCompilerIso({
      webpack,
      config: clientConfig,
      name: 'Client',
    ...
    const serverCompiler = createCompilerIso({
      webpack,
      config: serverConfig,
      name: 'Server',
    ...
    devServer.listen(devPort, HOST, err => {
      ...
      if (process.env.NODE_PATH) {
        console.log(
          chalk.yellow(
            'Setting NODE_PATH to resolve modules absolutely has been deprecated in favor of setting baseUrl in jsconfig.json (or tsconfig.json if you are using TypeScript) and will be removed in a future major release of react-starter.'
          )
        );
        console.log();
      }
